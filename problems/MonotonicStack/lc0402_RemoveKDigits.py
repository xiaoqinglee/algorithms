...
# 从左往右找到第一个位置 i（i>0）使得 Di<Di−1，并删去 Di−1；如果不存在，说明整个数字序列单调不降，删去最后一个数字即可。
#
...
class Solution:
    ...
    def removeKdigits(self, num: str, k: int) -> str:
        ...
        # 代码从这里开始不一样
        removed_digits: list[tuple[int, int]] = []

        # 栈底元素小，栈顶元素大于或等于栈底元素。构造这个栈的时候最先弹出的K个元素就是要删除的个元素。
        # 单调栈的栈元素是个二元组，（ElementValue，ElementIndex）
        remaining_digits: list[tuple[int, int]] = []
